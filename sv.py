...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.time1 = np.array([])
        self.magnitude1 = np.array([])
        self.time2 = np.array([])
        self.magnitude2 = np.array([])
        self.time3 = np.array([])
        self.magnitude3 = np.array([])
        self.transition=0
        self.zoomFactor=0
        self.counter = 0
        self.start = 0
    ...
    def viewData(self):
        ...
        if self.counter == 0:
            ...
            self.canv.axes.plot(self.time[self.start:self.end], self.magnitude[self.start:self.end])
            ...
            powerSpectrum, freqenciesFound, time, imageAxis = self.canv.axes2.specgram(self.magnitude, Fs = 1024 , NFFT = 32, noverlap = 16)
    ...
    def update_plot(self):
        ...
        if self.end == len(self.time) and self.mode == 'play':
            self.start = 0
            self.end = 500
            #draw
            self.canv.axes.cla()  # Clear the canvas.
        ...
        elif self.end != len(self.time)-1 and self.mode == 'play':
            self.start = self.end
            self.end += 500
            #draw
            self.canv.axes.cla()  # Clear the canvas.
    ...
    def signalDrawer(self,x,y,start,end):
        ...
        self.axs[1].specgram(self.magnitude, Fs = 1024 , NFFT = 32, noverlap = 16)
    ...
    def pdfGenerator(self):
        ...
        if self.transition == 1:
            graph1=self.signalDrawer(self.time1,self.magnitude1,self.start,self.end)
            pdf.savefig(graph1)
            print("Transition 1")
        elif self.transition == 2:
            graph1=self.signalDrawer(self.time1,self.magnitude1,self.start,self.end)
            graph2=self.signalDrawer(self.time2,self.magnitude2,self.start,self.end)
            pdf.savefig(graph1)
            pdf.savefig(graph2)
            ...
            graph1=self.signalDrawer(self.time1,self.magnitude1,self.start,self.end)
            graph2=self.signalDrawer(self.time2,self.magnitude2,self.start,self.end)
            graph3=self.signalDrawer(self.time3,self.magnitude3,self.start,self.end)
            pdf.savefig(graph1)
            pdf.savefig(graph2)
            pdf.savefig(graph3)
            # print(self.x1)
